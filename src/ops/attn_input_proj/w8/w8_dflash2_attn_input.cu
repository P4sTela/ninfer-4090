...
// Exact-T launchers keep the baseline small-T kernel's compile-time active-column contract.
...
constexpr auto make_small_launchers(std::index_sequence<I...>) {
    return std::array<Launch, sizeof...(I)>{&launch_small<1 + static_cast<int>(I)>...};
}
...
void w8_dflash2_attn_input_small_t_launch(const Tensor& x, const Weight& weight, Tensor& q,
    ...
    kSmallLaunchers[static_cast<std::size_t>(x.ne[1] - 1)](x, weight, q, k, v, stream);
}
