...
constexpr std::array<RouteSpec, 5> kDFlash2Routes{{
    ...
    {65, 96, W8LinearSwiGluScheduleId::DFlash2MmaR64C80K128},
    {97, kAnyCols, W8LinearSwiGluScheduleId::MmaR64C128},
}};
...
static_assert(catalog_is_closed(kRoutes), "W8 LinearSwiGLU routes must be exact and closed");
...
bool supported_shape(const W8LinearSwiGluProblem& problem) noexcept {
    const bool companion = problem.gate_up_rows == 12288 && problem.output_rows == 6144 &&
                           problem.k == 2048 && problem.padded_k == 2048;
    return companion || is_dflash2_shape(problem);
}
