...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    if constexpr (Variant::supports_dflash || target_supports_dflash2<Variant>()) {
        if (qwen3_6::is_masked_draft_backend(plan.speculative_backend)) {
    ...
    if constexpr (Variant::supports_dflash || target_supports_dflash2<Variant>()) {
        if (qwen3_6::is_masked_draft_backend(plan.speculative_backend)) {
            ...
            dflash.pending_features  = add_tensor(
                builder, DType::BF16,
                {DFlashConfig::feature_rows, static_cast<std::int32_t>(plan.draft_window + 1U),
...
void validate_target_options(DeviceContext& device, const EngineOptions& options) {
    ...
    case SpeculativeBackend::DFlash2:
        if constexpr (!target_supports_dflash2<Variant>()) {
        ...
        if (options.speculative.draft_tokens == 0 ||
            options.speculative.draft_tokens > kMaximumDFlashDraftTokens) {
            throw std::invalid_argument("DFlash2 draft window must be in [1,15]");
        }
        break;
    }
...
std::unique_ptr<SequencePlanImpl> build_sequence_candidate(const SequencePlanningInputs& inputs,
    ...
    // Masked-draft backends keep checkpoint state in their own cyclic mirror that only covers the
    // resident checkpoint; older ring entries could not rebuild it, so the ring stays off there.
    impl->turn_checkpoint_ring = qwen3_6::is_masked_draft_backend(inputs.speculative_backend)
                                     ? 0
                                     : inputs.turn_checkpoint_ring;
