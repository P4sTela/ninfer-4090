...
ProgramImplCore::ProgramImplCore(const LoadedModelData& model_in, const SequencePlanImpl& plan,
    ...
    : model(model_in), device(device_in), capacity(plan.capacity), kv_capacity(plan.kv_capacity),
      ...
      mtp_host(plan.speculative_backend == SpeculativeBackend::Mtp
                   ...
                   : std::nullopt),
      dflash_host(qwen3_6::is_masked_draft_backend(plan.speculative_backend)
    ...
    if (model.mtp.has_value() && (model.dflash.has_value() || model.dflash2.has_value())) {
        throw std::invalid_argument("MTP and masked-draft model views are mutually exclusive");
    }
    ...
    if (dflash.has_value() != qwen3_6::is_masked_draft_backend(speculative_backend)) {
        throw std::logic_error("masked-draft state does not match the frozen sequence plan");
    }
    ...
    if (io.dflash_prefill.has_value() !=
        qwen3_6::is_masked_draft_backend(speculative_backend)) {
        throw std::logic_error("masked-draft prefill scratch does not match the sequence plan");
    }
    if (io.dflash_decode.has_value() != qwen3_6::is_masked_draft_backend(speculative_backend)) {
        throw std::logic_error("masked-draft decode frame does not match the sequence plan");
    }
...
void ProgramImplCore::publish_checkpoint_drop(SequenceState& sequence,
    ...
    } else if (qwen3_6::is_masked_draft_backend(speculative_backend)) {
...
bool ProgramImplCore::compose_pressure_candidate(
    ...
    if (projection->source_state_fork_required &&
        ...
        if (residency == StateReplicaResidency::DeviceOnly ||
            ...
            if (qwen3_6::is_masked_draft_backend(speculative_backend)) {
    ...
    if (transaction.publish_shared) {
        ...
        const std::uint32_t backend_materialized =
            speculative_backend == SpeculativeBackend::Mtp
                ...
                : backend_kv_cache() != nullptr ? prefill.prompt_tokens : 0U;
...
runtime::ExecutionTiming ProgramImplCore::resolve_pending_raw(
    ...
    try {
        ...
        if (needs_hidden_correction) {
            ...
            } else if (qwen3_6::is_masked_draft_backend(speculative_backend) && io.dflash_decode) {
...
qwen3_6::PagedKVCache* ProgramImplCore::backend_kv_cache() noexcept {
    ...
    if (qwen3_6::is_masked_draft_backend(speculative_backend) && dflash && dflash->full) {
        return &*dflash->full;
    }
    return nullptr;
}
...
void ProgramImplCore::prepare_graphs() {
    ...
    if (speculative_backend == SpeculativeBackend::Mtp) {
        ...
        if (!dflash || !dflash->full || !backend_kv_addresses) {
            throw std::logic_error("DFlash graph capture has no full KV cache");
        }
...
void ProgramImplCore::enqueue_dflash_context_append(std::span<const std::uint32_t> lanes,
    ...
    if (!qwen3_6::is_masked_draft_backend(speculative_backend) || !dflash ||
        !io.dflash_decode || lanes.empty() || lanes.size() > max_concurrency ||
        starts.size() != lanes.size() || counts.size() != lanes.size()) {
...
ProgramImplCore::decode_dflash_batch(std::span<const std::uint32_t> lanes,
    ...
    nvtx::ScopedRange round_range(nvtx::Name::DecodeDFlashRound, nvtx::Category::DFlash,
                                  static_cast<std::uint64_t>(lanes.size()));
    runtime::ExecutionTimingRecorder timing(runtime::ExecutionTimingPhase::Submit, failed_timing);
    if (!qwen3_6::is_masked_draft_backend(speculative_backend) || !io.dflash_decode ||
        !dflash) {
        throw std::logic_error("masked-draft batch execution requires its backend");
    }
    if (lanes.empty() || lanes.size() > max_concurrency || budgets.size() != lanes.size()) {
    ...
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        ...
        if (request.lifecycle != Lifecycle::Active ||
            ...
            (backend_kv_cache() && !sequence.kv->backend) ||
            text_kv_addresses->bound_row(sequence.kv->text) < 0 ||
            (backend_kv_cache() && backend_kv_addresses->bound_row(*sequence.kv->backend) < 0) ||
            sequence.execution_frontier >= capacity ||
            sequence.text_kv_valid != sequence.execution_frontier ||
            sequence.dflash_context_frontier > sequence.execution_frontier ||
...
ProgramImplCore::decode_raw(std::span<const std::uint32_t> lanes,
    ...
    throw std::logic_error("unknown speculative backend");
}
