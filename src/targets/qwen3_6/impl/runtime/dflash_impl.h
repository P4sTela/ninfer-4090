...
template <class V>
consteval bool target_supports_dflash2() {
    if constexpr (requires { V::supports_dflash2; }) { return V::supports_dflash2; }
    return false;
}

template <class V>
consteval bool target_uses_dflash2_selector() {
    if constexpr (requires { V::DFlashConfig::coherent_selector; }) {
        return V::DFlashConfig::coherent_selector;
    }
    return false;
}
...
DFlashFeatureSink prefill_feature_sink_impl(PrefillContext& state,
    ...
    if constexpr (!V::supports_dflash && !target_supports_dflash2<V>()) {
        throw std::logic_error("masked draft feature capture is unavailable for this target");
    } else {
        require_dflash_state(state);
...
void append_context_impl(Context& state, const Tensor& features, const Tensor& positions,
    ...
    if constexpr (!V::supports_dflash && !target_supports_dflash2<V>()) {
        throw std::logic_error("masked draft context append is unavailable for this target");
    } else {
        using Config               = typename V::DFlashConfig;
        ...
        const int projected_width = target_uses_dflash2_selector<V>() ? local_width : width;
        const Tensor input        = target_uses_dflash2_selector<V>() && replace_local_window
        ...
        if constexpr (target_uses_dflash2_selector<V>()) {
            ops::linear(input.view({Config::feature_rows, projected_width * batch}),
                        ...
                        state.execution.device.stream);
            ops::rmsnorm(projected, state.execution.model.dflash2->context_norm,
        ...
        if constexpr (target_uses_dflash2_selector<V>()) {
            ...
            for (int layer = 0; layer < Config::layers; ++layer) {
                const auto& weights = state.execution.model.dflash2->layers[layer];
...
void propose_dflash2_batch(DFlashBatchContext& state, qwen3_6::DFlashDecodeState& frame, int batch,
    ...
    if constexpr (target_uses_dflash2_selector<V>()) {
        ...
        const qwen3_6::DFlash2Weights& weights = *state.execution.model.dflash2;
...
void propose_batch_impl(DFlashBatchContext& state, qwen3_6::DFlashDecodeState& frame,
    ...
    if constexpr (!V::supports_dflash && !target_supports_dflash2<V>()) {
        throw std::logic_error("masked draft proposal is unavailable for this target");
    } else if constexpr (target_uses_dflash2_selector<V>()) {
